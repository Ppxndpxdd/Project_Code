...
import os
import torch
import paho.mqtt.client as mqtt_client  # Updated import
import ssl
import uuid
...
class MaskTool:
    ...
    def is_point_near_polygon_point(self, point, polygon, threshold=None):
...
class ZoneIntersectionTracker:
    def __init__(self, config, show_result=True):
        ...
        # Generate a unique client ID or use the one from your working code
        self.client_id = '123456'

        # Initialize MQTT client with the unique client ID
        self.mqtt_client = mqtt_client.Client(
            client_id=self.client_id,
            protocol=mqtt_client.MQTTv311  # Explicitly specify the MQTT version
        )
        ...
        try:
            ...
            ca_cert_path = 'mask_tool\\code\\emqxsl-ca.crt'  # Use the same path as in your working code
            ...
            # Assign callbacks for debugging
    ...
    def publish_detection(self, detection_entry):
        try:
            # Convert all values in detection_entry to serializable types
            serializable_entry = {}
