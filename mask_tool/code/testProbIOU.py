...
class ZoneIntersectionTracker:
    ...
    def __init__(self, config: Dict[str, Any], show_result: bool = True):
        ...
        self.fps = 30  # Default FPS, to be updated in track_intersections
        ...
        logging.info(f"Using {'GPU' if self.device == 'cuda' else 'CPU'} for inference.")

        # Load the YOLO model
        self.model = YOLO(model_path).to(self.device)

        # Time thresholds for color changes
        self.total_duration = config.get('total_duration', 5)
        self.time_thresholds = [2 * self.total_duration / 3, self.total_duration]
        # MQTT configuration
    ...
    def publish_detection(self, detection_entry: DetectionEntry):
        ...
        # Define the Gaussian parameters for the bounding box

    def calculate_prob_iou(self, bbox: np.ndarray, polygon: np.ndarray) -> float:
        """Calculate the Probabilistic IoU (ProbIoU) between a Gaussian bounding box and a polygon."""
        # Define the Gaussian parameters for the bounding box
        x1, y1, x2, y2 = bbox
        center = ((x1 + x2) / 2, (y1 + y2) / 2)
        width = x2 - x1
        height = y2 - y1
        covariance = np.array([[width**2 / 16, 0], [0, height**2 / 16]])  # Example covariance matrix

        # Create a Gaussian distribution for the bounding box
        bbox_distribution = multivariate_normal(mean=center, cov=covariance)

        # Sample points from the polygon to estimate the overlap
        polygon_points = np.array(polygon, dtype=np.float32)
        poly_min_x, poly_min_y = np.min(polygon_points, axis=0)
        poly_max_x, poly_max_y = np.max(polygon_points, axis=0)

        # Generate a grid of points within the bounding box and polygon's range
        x_points = np.linspace(poly_min_x, poly_max_x, num=10)
        y_points = np.linspace(poly_min_y, poly_max_y, num=10)
        grid_points = np.array([(x, y) for x in x_points for y in y_points])

        # Mask points that are inside both the bounding box and the polygon
        bbox_mask = (grid_points[:, 0] >= x1) & (grid_points[:, 0] <= x2) & (grid_points[:, 1] >= y1) & (grid_points[:, 1] <= y2)
        poly_mask = np.array([cv2.pointPolygonTest(polygon, (p[0], p[1]), False) >= 0 for p in grid_points])
        intersection_points = grid_points[bbox_mask & poly_mask]

        # Calculate probability for intersection points
        intersection_probabilities = bbox_distribution.pdf(intersection_points)

        # Calculate ProbIoU: sum of probabilities at intersection points normalized by total bounding box area
        if len(intersection_points) > 0:
            prob_iou = np.sum(intersection_probabilities) / (bbox_distribution.pdf(center) * width * height)
        else:
            prob_iou = 0.0
    ...
    def track_intersections(self, video_path: str, frame_to_edit: int):
        """Tracks objects in the video and detects zone intersections using ProbIoU."""
        ...
        while cap.isOpened():
            ...
            for result in results:
                ...
                for i, (bbox, conf, class_id, track_id) in enumerate(zip(boxes.xyxy, boxes.conf.cpu().numpy(), boxes.cls.cpu().numpy(), track_ids)):
                    ...
                    if track_id not in self.tracked_objects:
                        self.tracked_objects[track_id] = {'class_id': int(class_id), 'zone_entries': []}

                    for zone_id, polygon in self.zones.items():
                        prob_iou = self.calculate_prob_iou(bbox_np, polygon)

                        if prob_iou > max_prob_iou:
                            max_prob_iou = prob_iou
                            intersecting_zone = zone_id if prob_iou >= 0.3 else None

                        zone_entry_exists = any(entry['zone_id'] == zone_id for entry in self.tracked_objects[track_id]['zone_entries'])
                        if prob_iou >= 0.3 and not zone_entry_exists:
                            entry = {'zone_id': int(zone_id), 'first_seen': float(timestamp), 'last_seen': None}
                            self.tracked_objects[track_id]['zone_entries'].append(entry)

                        elif prob_iou < 0.3 and zone_entry_exists:
                    ...
                    bbox_color = (0, 0, 255) if time_in_zone > self.time_thresholds[1] else (0, 165, 255) if time_in_zone > self.time_thresholds[0] else (0, 255, 0)
                    x1, y1, x2, y2 = bbox_np.astype(int)
                    cv2.rectangle(frame, (x1, y1), (x2, y2), bbox_color, 2)
                    label1 = f"ID: {track_id} ProbIoU: {max_prob_iou:.2f}"
