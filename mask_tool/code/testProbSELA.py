...
class ZoneIntersectionTracker:
    ...
    def intersects(self, bbox: np.ndarray, polygon: np.ndarray, threshold: float = 0.3) -> Tuple[bool, float]:
        """
        Determines if a bounding box intersects with a polygon based on IoU threshold.
        
        Args:
            bbox (np.ndarray): The bounding box coordinates [x1, y1, x2, y2].
            polygon (np.ndarray): The zone polygon coordinates.
            threshold (float): The IoU threshold for intersection.
        
        Returns:
            (bool, float): A tuple where the first element is True if intersection occurs
                        based on the threshold, and the second is the calculated IoU.
        """
        x1, y1, x2, y2 = bbox
        bbox_poly = np.array([[x1, y1], [x2, y1], [x2, y2], [x1, y2]], dtype=np.float32)

        # Calculate intersection between the bounding box polygon and the zone polygon
        intersection = cv2.intersectConvexConvex(bbox_poly, polygon.astype(np.float32))
        if intersection[1] is None:
            return False, 0.0
    ...
    def track_intersections(self, video_path: str, frame_to_edit: int):
        """Tracks objects in the video and detects zone intersections with Spatial Equilibrium Detection."""
        ...
        zone_performance = defaultdict(lambda: {"true_positives": 0, "false_positives": 0, "false_negatives": 0})
        ...
        while cap.isOpened():
            ...
            for result in results:
                ...
                for i, (bbox, conf, class_id, track_id) in enumerate(
                    ...
                    for zone_id, polygon in self.zones.items():
                        # Dynamic adjustment based on zone performance
                        current_zone_performance = zone_performance[zone_id]
                        sp = self.calculate_spatial_equilibrium_precision(current_zone_performance)

                        # Apply stricter threshold if the SP of the zone is low
                        iou_threshold = 0.3 if sp >= 0.7 else 0.5  # Adjust threshold based on SP
                        intersects, iou = self.intersects(bbox_np, polygon, threshold=iou_threshold)

                        if intersects:
                            intersecting_zones.append(zone_id)
                            detected_in_zone = True

                            # Update true positives if the detection was correctly assigned
                            if not any(entry['zone_id'] == zone_id for entry in self.tracked_objects[track_id]['zone_entries']):
                                current_zone_performance["true_positives"] += 1
                            zone_entry = {
                                'zone_id': int(zone_id),
                                'first_seen': float(timestamp),
                                'last_seen': None
                            }
                            self.tracked_objects[track_id]['zone_entries'].append(zone_entry)
                        ...
                        # Handle exit
                        elif any(entry['zone_id'] == zone_id for entry in self.tracked_objects[track_id]['zone_entries']):
                            for entry in self.tracked_objects[track_id]['zone_entries']:
                                if entry['zone_id'] == zone_id and entry['last_seen'] is None:
                                    ...
                                    # Update false negatives if the object should be in the zone
                                    current_zone_performance["false_negatives"] += 1

                    if not detected_in_zone:
                        # Update false positives for zones where detection was mistakenly assigned
                        zone_performance[zone_id]["false_positives"] += 1
                    ...
                    if time_in_zone > self.total_duration:
                        bbox_color = (0, 0, 255)
    ...
    def calculate_spatial_equilibrium_precision(self, performance: Dict[str, int]) -> float:
        """Calculates the Spatial Equilibrium Precision (SP) for a zone."""
        tp = performance["true_positives"]
        fp = performance["false_positives"]
        fn = performance["false_negatives"]
        if tp + fp + fn == 0:
            return 1.0
        return tp / (tp + fp + fn)
