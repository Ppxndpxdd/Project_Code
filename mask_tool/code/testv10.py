...
class MaskTool:
    ...
    def run(self):
        ...
        while True:
            ...
            if key == ord('s'):
                self.mask_positions.to_csv('config\\mask_positions.csv', index=True)
...
class ZoneIntersectionTracker:
    def __init__(self, model_path, mask_csv_path, tracker_config="bytetrack.yaml", device='gpu'):
            ...
            for result in results:
                ...
                for i, (bbox, conf, class_id, track_id) in enumerate(zip(boxes.xyxy, boxes.conf.cpu().numpy(), boxes.cls.cpu().numpy(), track_ids)):
                    ...
                    for zone_id, polygon in self.zones.items():
                        ...
                        if intersects:
                            self.object_zone_timers[track_id][zone_id] += frame_time
                        else:
                            self.object_zone_timers[track_id][zone_id] = 0

                    time_in_zone = max(self.object_zone_timers[track_id].values()) if self.object_zone_timers[track_id] else 0
                    ...
                    label2 = f"Time: {time_in_zone:.1f}s Zone: {intersecting_zone}"
                    ...
                    print(f"Object {track_id}: bbox={bbox_np}, IoU={max_iou:.2f}, time_in_zone={time_in_zone:.1f}, zone={intersecting_zone}")

                    # Log intersection if time > 3 seconds
                    if time_in_zone > 3 and intersecting_zone is not None:
                        ...
                        if intersecting_zone not in self.tracked_objects[track_id]['zone_entries']:
                            self.tracked_objects[track_id]['zone_entries'].append(intersecting_zone)
                            self.detection_log.append({
                                'frame_id': frame_id,
                                'object_id': track_id,
                                'class_id': class_id,
                                'confidence': conf,
                                'zone_id': intersecting_zone,
...
if __name__ == "__main__":
    ...
    model_path = 'YoLo\\model\\yolov10n.onnx'
    frame_to_edit = 1
    ...
    tracker = ZoneIntersectionTracker(model_path, mask_csv_path, tracker_config="bytetrack.yaml",device=0)
