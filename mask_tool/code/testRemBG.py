            ...
            while cap.isOpened():
                ...
                for result in results:
                    ...
                    for i, (bbox, conf, class_id, track_id) in enumerate(
                        ...
                        object_region_no_bg = remove(cv2.imencode('.png', object_region)[1].tobytes())
                        object_no_bg = cv2.imdecode(np.frombuffer(object_region_no_bg, np.uint8), cv2.IMREAD_UNCHANGED)

                        # Compute tight bounding box
                        gray = cv2.cvtColor(object_no_bg, cv2.COLOR_BGRA2GRAY)
                        _, thresh = cv2.threshold(gray, 1, 255, cv2.THRESH_BINARY)
                        contours, _ = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                        
                        # Find tight bounding box around non-background pixels
                        if contours:
                            x, y, w, h = cv2.boundingRect(contours[0])
                            tight_bbox = (x1 + x, y1 + y, x1 + x + w, y1 + y + h)
                        else:
                            tight_bbox = (x1, y1, x2, y2)  # Fallback if no contours are found

                        # Draw tight bounding box
                        cv2.rectangle(frame, (tight_bbox[0], tight_bbox[1]), (tight_bbox[2], tight_bbox[3]), (0, 255, 0), 2)
                        label = f"ID: {track_id} Conf: {conf:.2f}"
                        cv2.putText(frame, label, (tight_bbox[0], tight_bbox[1] - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
